...
def get_cpu_info() -> dict:
    ...
    avg_core_usage = sum(core_usages) / len(core_usages)
    ...
    if "Intel" in platform.processor() and platform.system() == "Linux":
        try:
            # Run the sensors command to get temperature information
            result = subprocess.run(['sensors'], capture_output=True, text=True, check=True)
            output = result.stdout

            # Extract temperatures for each core
            core_temperatures = []
            ...
            if core_temperatures:
                ...
                avg_temp = sum(core_temperatures) / len(core_temperatures)
